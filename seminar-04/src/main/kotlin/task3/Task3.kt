 ...
 * of two child sets.
 */
data class SetIntersection<S>(val size: Int, val lowerBound: S?, val upperBound: S?): CostElement {
...
fun main() {
    val set1 = SetIntersection(10, 5, 15)
    val set2 = SetIntersection(20, 10, 25)
    val set3 = SetIntersection(30, 0, 5)

    val node1 = TreeNode(set1)
    val node2 = TreeNode(set2)
    val node3 = TreeNode(set3)

    node1.left = node2
    node1.right = node3

    println("Estimated intersection size: ${estimateIntersectionSize(node1)}")
}
